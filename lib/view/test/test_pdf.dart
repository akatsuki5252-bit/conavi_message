            ...
            onPressed: () => Navigator.push(
              ...
              MaterialPageRoute<dynamic>(
                builder: (_) => const PDFViewerCachedFromUrl(
                  url: 'https://dev.conavi.net/api/upload/file.php?message_id=212&app_token=3dd5eab130289199a9211aa6eb0de142',
                ),
              ),
            ),
...
class PDFViewerFromUrl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        color: Colors.grey,
        child: Scaffold()/*const PDF().fromUrl(
          url,
...
class PDFViewerCachedFromUrl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Scaffold()/*const PDF().cachedFromUrl(
        url,
        placeholder: (double progress) => Center(child: Text('$progress %')),
        errorWidget: (dynamic error) => Center(child: Text(error.toString())),
      ),*/
